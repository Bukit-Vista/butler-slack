...
} = require("./../../utils/modal/conversation_modal");
...
async function hideMessage(payload) {
    const statusValue =
        payload.message.blocks[payload.message.blocks.length - 1].elements[0]
            .value;
    ...
    const slackRes = await api.callAPIMethod("chat.update", {
        channel: "C04MW37RD5G",
...
async function ignoreMessage(payload) {
    ...
    for (let i = 0; i < res.messages.length - 1; i++) {
        if (
            res.messages[i].bot_id &&
            res.messages[i].text === "This content can't be displayed."
        ) {
